...
export class PlaceDAO{
    ...
    public async getByBatiment(idBatiment: string): Promise<Array<Place>> {

        let lesPlaces = await this.getAll();
        lesPlaces = lesPlaces.filter(elt => elt.etage.batiment.idBatiment===idBatiment);
        return lesPlaces;
    ...
    async nombrebyBatiment(id:string) : Promise<number> {
        const place = (await this.getByBatiment(id));
        return place.length;
    }

    async nombrebyEtage(id:string) : Promise<number> {
        const place = await this.getByEtage(id);
        return place.length;
    }
